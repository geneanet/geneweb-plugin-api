...
let _visibility_piqi_type = Piqirun_ext.find_piqi_type "api_saisie_read/visibility"
...
let parse_visibility ?opts x (format :Piqirun_ext.input_format) =
  ...
  let buf = Piqirun.init_from_string x_pb in
  Api_saisie_read_piqi.parse_visibility buf
...
let gen_visibility ?opts x (format :Piqirun_ext.output_format) =
  let buf = Api_saisie_read_piqi.gen_visibility x in
  let x_pb = Piqirun.to_string buf in
...
let print_fiche_parameters ?opts x =
  Stdlib.print_endline (gen_fiche_parameters x `piq ?opts)
let prerr_fiche_parameters ?opts x =
  Stdlib.prerr_endline (gen_fiche_parameters x `piq ?opts)
...
let print_witness_fiche_event ?opts x =
  Stdlib.print_endline (gen_witness_fiche_event x `piq ?opts)
let prerr_witness_fiche_event ?opts x =
  Stdlib.prerr_endline (gen_witness_fiche_event x `piq ?opts)
...
let print_fiche_event ?opts x =
  Stdlib.print_endline (gen_fiche_event x `piq ?opts)
let prerr_fiche_event ?opts x =
  Stdlib.prerr_endline (gen_fiche_event x `piq ?opts)

let print_person_tree ?opts x =
  Stdlib.print_endline (gen_person_tree x `piq ?opts)
let prerr_person_tree ?opts x =
  Stdlib.prerr_endline (gen_person_tree x `piq ?opts)
...
let print_relation_fiche_person ?opts x =
  Stdlib.print_endline (gen_relation_fiche_person x `piq ?opts)
let prerr_relation_fiche_person ?opts x =
  Stdlib.prerr_endline (gen_relation_fiche_person x `piq ?opts)

let print_event_witness ?opts x =
  Stdlib.print_endline (gen_event_witness x `piq ?opts)
let prerr_event_witness ?opts x =
  Stdlib.prerr_endline (gen_event_witness x `piq ?opts)

let print_event_fiche_witness ?opts x =
  Stdlib.print_endline (gen_event_fiche_witness x `piq ?opts)
let prerr_event_fiche_witness ?opts x =
  Stdlib.prerr_endline (gen_event_fiche_witness x `piq ?opts)
...
let print_person_type ?opts x =
  Stdlib.print_endline (gen_person_type x `piq ?opts)
let prerr_person_type ?opts x =
  Stdlib.prerr_endline (gen_person_type x `piq ?opts)

let print_fiche_person ?opts x =
  Stdlib.print_endline (gen_fiche_person x `piq ?opts)
let prerr_fiche_person ?opts x =
  Stdlib.prerr_endline (gen_fiche_person x `piq ?opts)
...
let print_fiche_family ?opts x =
  Stdlib.print_endline (gen_fiche_family x `piq ?opts)
let prerr_fiche_family ?opts x =
  Stdlib.prerr_endline (gen_fiche_family x `piq ?opts)
...
let print_graph_tree ?opts x =
  Stdlib.print_endline (gen_graph_tree x `piq ?opts)
let prerr_graph_tree ?opts x =
  Stdlib.prerr_endline (gen_graph_tree x `piq ?opts)

let print_graph_tree_params ?opts x =
  Stdlib.print_endline (gen_graph_tree_params x `piq ?opts)
let prerr_graph_tree_params ?opts x =
  Stdlib.prerr_endline (gen_graph_tree_params x `piq ?opts)

let print_identifier_person ?opts x =
  Stdlib.print_endline (gen_identifier_person x `piq ?opts)
let prerr_identifier_person ?opts x =
  Stdlib.prerr_endline (gen_identifier_person x `piq ?opts)

let print_nb_ancestors ?opts x =
  Stdlib.print_endline (gen_nb_ancestors x `piq ?opts)
let prerr_nb_ancestors ?opts x =
  Stdlib.prerr_endline (gen_nb_ancestors x `piq ?opts)
...
let print_burial_type ?opts x =
  Stdlib.print_endline (gen_burial_type x `piq ?opts)
let prerr_burial_type ?opts x =
  Stdlib.prerr_endline (gen_burial_type x `piq ?opts)
...
let print_event_type ?opts x =
  Stdlib.print_endline (gen_event_type x `piq ?opts)
let prerr_event_type ?opts x =
  Stdlib.prerr_endline (gen_event_type x `piq ?opts)
