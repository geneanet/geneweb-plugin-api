...
let _created_person_piqi_type = Piqirun_ext.find_piqi_type "api_saisie_write/created-person"
...
let parse_created_person ?opts x (format :Piqirun_ext.input_format) =
  let x_pb = Piqirun_ext.convert _created_person_piqi_type format `pb x ?opts in
  let buf = Piqirun.init_from_string x_pb in
  Api_saisie_write_piqi.parse_created_person buf
...
let gen_created_person ?opts x (format :Piqirun_ext.output_format) =
  let buf = Api_saisie_write_piqi.gen_created_person x in
  let x_pb = Piqirun.to_string buf in
  Piqirun_ext.convert _created_person_piqi_type `pb format x_pb ?opts
...
let print_person_search ?opts x =
  Stdlib.print_endline (gen_person_search x `piq ?opts)
let prerr_person_search ?opts x =
  Stdlib.prerr_endline (gen_person_search x `piq ?opts)

let print_simple_person ?opts x =
  Stdlib.print_endline (gen_simple_person x `piq ?opts)
let prerr_simple_person ?opts x =
  Stdlib.prerr_endline (gen_simple_person x `piq ?opts)

let print_witness_event ?opts x =
  Stdlib.print_endline (gen_witness_event x `piq ?opts)
let prerr_witness_event ?opts x =
  Stdlib.prerr_endline (gen_witness_event x `piq ?opts)

let print_event ?opts x =
  Stdlib.print_endline (gen_event x `piq ?opts)
let prerr_event ?opts x =
  Stdlib.prerr_endline (gen_event x `piq ?opts)

let print_relation_person ?opts x =
  Stdlib.print_endline (gen_relation_person x `piq ?opts)
let prerr_relation_person ?opts x =
  Stdlib.prerr_endline (gen_relation_person x `piq ?opts)

let print_was_witness ?opts x =
  Stdlib.print_endline (gen_was_witness x `piq ?opts)
let prerr_was_witness ?opts x =
  Stdlib.prerr_endline (gen_was_witness x `piq ?opts)

let print_person_search_info ?opts x =
  Stdlib.print_endline (gen_person_search_info x `piq ?opts)
let prerr_person_search_info ?opts x =
  Stdlib.prerr_endline (gen_person_search_info x `piq ?opts)

let print_person_link ?opts x =
  Stdlib.print_endline (gen_person_link x `piq ?opts)
let prerr_person_link ?opts x =
  Stdlib.prerr_endline (gen_person_link x `piq ?opts)

let print_witness ?opts x =
  Stdlib.print_endline (gen_witness x `piq ?opts)
let prerr_witness ?opts x =
  Stdlib.prerr_endline (gen_witness x `piq ?opts)

let print_fevent ?opts x =
  Stdlib.print_endline (gen_fevent x `piq ?opts)
let prerr_fevent ?opts x =
  Stdlib.prerr_endline (gen_fevent x `piq ?opts)
...
let print_pevent ?opts x =
  Stdlib.print_endline (gen_pevent x `piq ?opts)
let prerr_pevent ?opts x =
  Stdlib.prerr_endline (gen_pevent x `piq ?opts)
...
let print_family ?opts x =
  Stdlib.print_endline (gen_family x `piq ?opts)
let prerr_family ?opts x =
  Stdlib.prerr_endline (gen_family x `piq ?opts)

let print_create_conflict ?opts x =
  Stdlib.print_endline (gen_create_conflict x `piq ?opts)
let prerr_create_conflict ?opts x =
  Stdlib.prerr_endline (gen_create_conflict x `piq ?opts)

let print_created_person ?opts x =
  Stdlib.print_endline (gen_created_person x `piq ?opts)
let prerr_created_person ?opts x =
  Stdlib.prerr_endline (gen_created_person x `piq ?opts)
...
let print_index_person ?opts x =
  Stdlib.print_endline (gen_index_person x `piq ?opts)
let prerr_index_person ?opts x =
  Stdlib.prerr_endline (gen_index_person x `piq ?opts)

let print_index_family ?opts x =
  Stdlib.print_endline (gen_index_family x `piq ?opts)
let prerr_index_family ?opts x =
  Stdlib.prerr_endline (gen_index_family x `piq ?opts)

let print_index_person_and_family ?opts x =
  Stdlib.print_endline (gen_index_person_and_family x `piq ?opts)
let prerr_index_person_and_family ?opts x =
  Stdlib.prerr_endline (gen_index_person_and_family x `piq ?opts)

let print_family_spouse ?opts x =
  Stdlib.print_endline (gen_family_spouse x `piq ?opts)
let prerr_family_spouse ?opts x =
  Stdlib.prerr_endline (gen_family_spouse x `piq ?opts)

let print_add_child_request ?opts x =
  Stdlib.print_endline (gen_add_child_request x `piq ?opts)
let prerr_add_child_request ?opts x =
  Stdlib.prerr_endline (gen_add_child_request x `piq ?opts)

let print_add_child ?opts x =
  Stdlib.print_endline (gen_add_child x `piq ?opts)
let prerr_add_child ?opts x =
  Stdlib.prerr_endline (gen_add_child x `piq ?opts)

let print_add_child_ok ?opts x =
  Stdlib.print_endline (gen_add_child_ok x `piq ?opts)
let prerr_add_child_ok ?opts x =
  Stdlib.prerr_endline (gen_add_child_ok x `piq ?opts)

let print_add_parents ?opts x =
  Stdlib.print_endline (gen_add_parents x `piq ?opts)
let prerr_add_parents ?opts x =
  Stdlib.prerr_endline (gen_add_parents x `piq ?opts)

let print_add_parents_ok ?opts x =
  Stdlib.print_endline (gen_add_parents_ok x `piq ?opts)
let prerr_add_parents_ok ?opts x =
  Stdlib.prerr_endline (gen_add_parents_ok x `piq ?opts)

let print_add_family ?opts x =
  Stdlib.print_endline (gen_add_family x `piq ?opts)
let prerr_add_family ?opts x =
  Stdlib.prerr_endline (gen_add_family x `piq ?opts)

let print_add_family_ok ?opts x =
  Stdlib.print_endline (gen_add_family_ok x `piq ?opts)
let prerr_add_family_ok ?opts x =
  Stdlib.prerr_endline (gen_add_family_ok x `piq ?opts)

let print_edit_family_request ?opts x =
  Stdlib.print_endline (gen_edit_family_request x `piq ?opts)
let prerr_edit_family_request ?opts x =
  Stdlib.prerr_endline (gen_edit_family_request x `piq ?opts)

let print_edit_family ?opts x =
  Stdlib.print_endline (gen_edit_family x `piq ?opts)
let prerr_edit_family ?opts x =
  Stdlib.prerr_endline (gen_edit_family x `piq ?opts)

let print_edit_family_ok ?opts x =
  Stdlib.print_endline (gen_edit_family_ok x `piq ?opts)
let prerr_edit_family_ok ?opts x =
  Stdlib.prerr_endline (gen_edit_family_ok x `piq ?opts)

let print_add_sibling_request ?opts x =
  Stdlib.print_endline (gen_add_sibling_request x `piq ?opts)
let prerr_add_sibling_request ?opts x =
  Stdlib.prerr_endline (gen_add_sibling_request x `piq ?opts)

let print_add_sibling ?opts x =
  Stdlib.print_endline (gen_add_sibling x `piq ?opts)
let prerr_add_sibling ?opts x =
  Stdlib.prerr_endline (gen_add_sibling x `piq ?opts)

let print_add_sibling_ok ?opts x =
  Stdlib.print_endline (gen_add_sibling_ok x `piq ?opts)
let prerr_add_sibling_ok ?opts x =
  Stdlib.prerr_endline (gen_add_sibling_ok x `piq ?opts)

let print_add_first_fam ?opts x =
  Stdlib.print_endline (gen_add_first_fam x `piq ?opts)
let prerr_add_first_fam ?opts x =
  Stdlib.prerr_endline (gen_add_first_fam x `piq ?opts)

let print_auto_complete ?opts x =
  Stdlib.print_endline (gen_auto_complete x `piq ?opts)
let prerr_auto_complete ?opts x =
  Stdlib.prerr_endline (gen_auto_complete x `piq ?opts)

let print_auto_complete_result ?opts x =
  Stdlib.print_endline (gen_auto_complete_result x `piq ?opts)
let prerr_auto_complete_result ?opts x =
  Stdlib.prerr_endline (gen_auto_complete_result x `piq ?opts)

let print_person_search_list_params ?opts x =
  Stdlib.print_endline (gen_person_search_list_params x `piq ?opts)
let prerr_person_search_list_params ?opts x =
  Stdlib.prerr_endline (gen_person_search_list_params x `piq ?opts)

let print_person_search_list ?opts x =
  Stdlib.print_endline (gen_person_search_list x `piq ?opts)
let prerr_person_search_list ?opts x =
  Stdlib.prerr_endline (gen_person_search_list x `piq ?opts)

let print_transl_calendar ?opts x =
  Stdlib.print_endline (gen_transl_calendar x `piq ?opts)
let prerr_transl_calendar ?opts x =
  Stdlib.prerr_endline (gen_transl_calendar x `piq ?opts)

let print_config_transl_calendar ?opts x =
  Stdlib.print_endline (gen_config_transl_calendar x `piq ?opts)
let prerr_config_transl_calendar ?opts x =
  Stdlib.prerr_endline (gen_config_transl_calendar x `piq ?opts)

let print_transl_witness_type ?opts x =
  Stdlib.print_endline (gen_transl_witness_type x `piq ?opts)
let prerr_transl_witness_type ?opts x =
  Stdlib.prerr_endline (gen_transl_witness_type x `piq ?opts)

let print_config_transl_witness_type ?opts x =
  Stdlib.print_endline (gen_config_transl_witness_type x `piq ?opts)
let prerr_config_transl_witness_type ?opts x =
  Stdlib.prerr_endline (gen_config_transl_witness_type x `piq ?opts)

let print_transl_precision ?opts x =
  Stdlib.print_endline (gen_transl_precision x `piq ?opts)
let prerr_transl_precision ?opts x =
  Stdlib.prerr_endline (gen_transl_precision x `piq ?opts)

let print_config_transl_precision ?opts x =
  Stdlib.print_endline (gen_config_transl_precision x `piq ?opts)
let prerr_config_transl_precision ?opts x =
  Stdlib.prerr_endline (gen_config_transl_precision x `piq ?opts)

let print_transl_death_type ?opts x =
  Stdlib.print_endline (gen_transl_death_type x `piq ?opts)
let prerr_transl_death_type ?opts x =
  Stdlib.prerr_endline (gen_transl_death_type x `piq ?opts)

let print_config_transl_death_type ?opts x =
  Stdlib.print_endline (gen_config_transl_death_type x `piq ?opts)
let prerr_config_transl_death_type ?opts x =
  Stdlib.prerr_endline (gen_config_transl_death_type x `piq ?opts)

let print_transl_relation_parent_type ?opts x =
  Stdlib.print_endline (gen_transl_relation_parent_type x `piq ?opts)
let prerr_transl_relation_parent_type ?opts x =
  Stdlib.prerr_endline (gen_transl_relation_parent_type x `piq ?opts)

let print_config_transl_relation_parent_type ?opts x =
  Stdlib.print_endline (gen_config_transl_relation_parent_type x `piq ?opts)
let prerr_config_transl_relation_parent_type ?opts x =
  Stdlib.prerr_endline (gen_config_transl_relation_parent_type x `piq ?opts)

let print_transl_fevent_name ?opts x =
  Stdlib.print_endline (gen_transl_fevent_name x `piq ?opts)
let prerr_transl_fevent_name ?opts x =
  Stdlib.prerr_endline (gen_transl_fevent_name x `piq ?opts)

let print_config_transl_fevent_name ?opts x =
  Stdlib.print_endline (gen_config_transl_fevent_name x `piq ?opts)
let prerr_config_transl_fevent_name ?opts x =
  Stdlib.prerr_endline (gen_config_transl_fevent_name x `piq ?opts)

let print_transl_pevent_name ?opts x =
  Stdlib.print_endline (gen_transl_pevent_name x `piq ?opts)
let prerr_transl_pevent_name ?opts x =
  Stdlib.prerr_endline (gen_transl_pevent_name x `piq ?opts)

let print_config_transl_pevent_name ?opts x =
  Stdlib.print_endline (gen_config_transl_pevent_name x `piq ?opts)
let prerr_config_transl_pevent_name ?opts x =
  Stdlib.prerr_endline (gen_config_transl_pevent_name x `piq ?opts)

let print_transl_access ?opts x =
  Stdlib.print_endline (gen_transl_access x `piq ?opts)
let prerr_transl_access ?opts x =
  Stdlib.prerr_endline (gen_transl_access x `piq ?opts)

let print_config_transl_access ?opts x =
  Stdlib.print_endline (gen_config_transl_access x `piq ?opts)
let prerr_config_transl_access ?opts x =
  Stdlib.prerr_endline (gen_config_transl_access x `piq ?opts)

let print_transl_update_warning_js ?opts x =
  Stdlib.print_endline (gen_transl_update_warning_js x `piq ?opts)
let prerr_transl_update_warning_js ?opts x =
  Stdlib.prerr_endline (gen_transl_update_warning_js x `piq ?opts)

let print_config_transl_update_warning_js ?opts x =
  Stdlib.print_endline (gen_config_transl_update_warning_js x `piq ?opts)
let prerr_config_transl_update_warning_js ?opts x =
  Stdlib.prerr_endline (gen_config_transl_update_warning_js x `piq ?opts)

let print_transl_short_greg_month ?opts x =
  Stdlib.print_endline (gen_transl_short_greg_month x `piq ?opts)
let prerr_transl_short_greg_month ?opts x =
  Stdlib.prerr_endline (gen_transl_short_greg_month x `piq ?opts)

let print_config_transl_short_greg_month ?opts x =
  Stdlib.print_endline (gen_config_transl_short_greg_month x `piq ?opts)
let prerr_config_transl_short_greg_month ?opts x =
  Stdlib.prerr_endline (gen_config_transl_short_greg_month x `piq ?opts)

let print_transl_french_month ?opts x =
  Stdlib.print_endline (gen_transl_french_month x `piq ?opts)
let prerr_transl_french_month ?opts x =
  Stdlib.prerr_endline (gen_transl_french_month x `piq ?opts)

let print_config_transl_french_month ?opts x =
  Stdlib.print_endline (gen_config_transl_french_month x `piq ?opts)
let prerr_config_transl_french_month ?opts x =
  Stdlib.prerr_endline (gen_config_transl_french_month x `piq ?opts)

let print_transl_hebrew_month ?opts x =
  Stdlib.print_endline (gen_transl_hebrew_month x `piq ?opts)
let prerr_transl_hebrew_month ?opts x =
  Stdlib.prerr_endline (gen_transl_hebrew_month x `piq ?opts)

let print_config_transl_hebrew_month ?opts x =
  Stdlib.print_endline (gen_config_transl_hebrew_month x `piq ?opts)
let prerr_config_transl_hebrew_month ?opts x =
  Stdlib.prerr_endline (gen_config_transl_hebrew_month x `piq ?opts)

let print_config ?opts x =
  Stdlib.print_endline (gen_config x `piq ?opts)
let prerr_config ?opts x =
  Stdlib.prerr_endline (gen_config x `piq ?opts)

let print_sosa ?opts x =
  Stdlib.print_endline (gen_sosa x `piq ?opts)
let prerr_sosa ?opts x =
  Stdlib.prerr_endline (gen_sosa x `piq ?opts)
...
let print_relation_type ?opts x =
  Stdlib.print_endline (gen_relation_type x `piq ?opts)
let prerr_relation_type ?opts x =
  Stdlib.prerr_endline (gen_relation_type x `piq ?opts)

let print_create_or_link ?opts x =
  Stdlib.print_endline (gen_create_or_link x `piq ?opts)
let prerr_create_or_link ?opts x =
  Stdlib.prerr_endline (gen_create_or_link x `piq ?opts)
...
let print_access ?opts x =
  Stdlib.print_endline (gen_access x `piq ?opts)
let prerr_access ?opts x =
  Stdlib.prerr_endline (gen_access x `piq ?opts)

let print_update_warning_js ?opts x =
  Stdlib.print_endline (gen_update_warning_js x `piq ?opts)
let prerr_update_warning_js ?opts x =
  Stdlib.prerr_endline (gen_update_warning_js x `piq ?opts)

let print_person_or_family ?opts x =
  Stdlib.print_endline (gen_person_or_family x `piq ?opts)
let prerr_person_or_family ?opts x =
  Stdlib.prerr_endline (gen_person_or_family x `piq ?opts)

let print_auto_complete_place_field ?opts x =
  Stdlib.print_endline (gen_auto_complete_place_field x `piq ?opts)
let prerr_auto_complete_place_field ?opts x =
  Stdlib.prerr_endline (gen_auto_complete_place_field x `piq ?opts)

let print_auto_complete_field ?opts x =
  Stdlib.print_endline (gen_auto_complete_field x `piq ?opts)
let prerr_auto_complete_field ?opts x =
  Stdlib.prerr_endline (gen_auto_complete_field x `piq ?opts)

let print_short_greg_month ?opts x =
  Stdlib.print_endline (gen_short_greg_month x `piq ?opts)
let prerr_short_greg_month ?opts x =
  Stdlib.prerr_endline (gen_short_greg_month x `piq ?opts)

let print_french_month ?opts x =
  Stdlib.print_endline (gen_french_month x `piq ?opts)
let prerr_french_month ?opts x =
  Stdlib.prerr_endline (gen_french_month x `piq ?opts)

let print_hebrew_month ?opts x =
  Stdlib.print_endline (gen_hebrew_month x `piq ?opts)
let prerr_hebrew_month ?opts x =
  Stdlib.prerr_endline (gen_hebrew_month x `piq ?opts)
