...
let print_data ?opts x =
  Stdlib.print_endline (gen_data x `piq ?opts)
let prerr_data ?opts x =
  Stdlib.prerr_endline (gen_data x `piq ?opts)

let print_data_l ?opts x =
  Stdlib.print_endline (gen_data_l x `piq ?opts)
let prerr_data_l ?opts x =
  Stdlib.prerr_endline (gen_data_l x `piq ?opts)

let print_stat ?opts x =
  Stdlib.print_endline (gen_stat x `piq ?opts)
let prerr_stat ?opts x =
  Stdlib.prerr_endline (gen_stat x `piq ?opts)

let print_stats ?opts x =
  Stdlib.print_endline (gen_stats x `piq ?opts)
let prerr_stats ?opts x =
  Stdlib.prerr_endline (gen_stats x `piq ?opts)

let print_stats_params ?opts x =
  Stdlib.print_endline (gen_stats_params x `piq ?opts)
let prerr_stats_params ?opts x =
  Stdlib.prerr_endline (gen_stats_params x `piq ?opts)
...
let print_serie ?opts x =
  Stdlib.print_endline (gen_serie x `piq ?opts)
let prerr_serie ?opts x =
  Stdlib.prerr_endline (gen_serie x `piq ?opts)
