    ...
    | `place ->
      let acc =
        Gwdb.Collection.fold
          (fun acc p ->
             List.fold_left
               (fun acc e -> add acc (Event.get_place e)) acc (get_pevents p) )
          IstrSet.empty (Gwdb.persons base)
      in
      Gwdb.Collection.fold
        (fun acc f -> List.fold_left (fun acc e -> add acc (Event.get_place e)) acc (get_fevents f) )
      ...
      let acc =
        Gwdb.Collection.fold
          (fun acc p ->
             ...
             List.fold_left (fun acc e -> add acc (Event.get_src e)) acc (get_pevents p) )
          IstrSet.empty
          (Gwdb.persons base)
      in
      Gwdb.Collection.fold
        (fun acc f ->
           ...
           List.fold_left (fun acc e -> add acc (Event.get_src e) acc (get_fevents f) )
        acc
        (Gwdb.families base)
  in
