...
let log_mod_fam mod_family =
  let _ =
    let f_occ o = Option.value ~default:(Int32.of_int (-1)) o |> Int32.to_string in

    let f_link = function
      |`link -> "link"
      | `create -> "create"
      | `create_default_occ -> "create_dtf_occ"
    in      
    ...
    let fath_occ = f_occ father.Mwrite.Person.occ in
    let moth_occ = f_occ mother.Mwrite.Person.occ in

    let fath_link = f_link father.Mwrite.Person.create_link in
    let moth_link = f_link mother.Mwrite.Person.create_link in

    let ifath = father.Mwrite.Person.index |> Int32.to_string in
    let imoth = mother.Mwrite.Person.index |> Int32.to_string in


    (!GWPARAM.syslog) `LOG_ERR @@ "IP VALUES RECIEVED : " ^ ifath ^ "  " ^ imoth;
    (!GWPARAM.syslog) `LOG_ERR @@ "OCC VALUES RECIEVED : " ^ fath_occ ^ "  " ^ moth_occ;
    (!GWPARAM.syslog) `LOG_ERR @@ "LNK VALUES RECIEVED : " ^ fath_link ^ "  " ^ moth_link
  in
  ()
...
let print_add_family_ok conf base =
  ...
  log_mod_fam mod_family;
(*  
  let father = mod_family.Mwrite.Family.father in
  let fath_occ = father.Mwrite.Person.occ in
  let mod_family = if fath_occ = None then mod_family
    else {mod_family with Mwrite.Family.father = {father with Mwrite.Person.create_link = `link}}
  in
*)
